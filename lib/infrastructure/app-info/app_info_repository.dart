...
class AppInfoRepository implements IAppInfoRepository {
  ...
  @override
  Future<Either<AppInfoFailure, AppOperationUrl>> getOperationUrl() async {
    final appOerationUrlValue = firebaseRemoteConfig.getValue(AppInfoRepository.remoteConfigKey).asString();
    final appOerationJson = json.decode(appOerationUrlValue) as Map<String, dynamic>;
    debugPrint("FULTTER_CORE :: getOperationUrl :: success json -> $appOerationJson ");
    final appOperationUrl = AppOperationUrlDto.fromJson(appOerationJson).toDomain();
    final PackageInfo packageInfo = await PackageInfo.fromPlatform();
    debugPrint("FULTTER_CORE :: getOperationUrl :: success -> ${appOperationUrl.toString()}  //  ${packageInfo.version}");
    return right(appOperationUrl.copyWith(appVersion: appOperationUrl.appVersion.copyWith(current: VersionVO(packageInfo.version))));
  }
}
